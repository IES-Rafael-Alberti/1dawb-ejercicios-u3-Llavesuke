...
def calcular_puntos(cartas_jugador: list) -> tuple:
...
def valor_carta(carta: tuple, valor_minimo = True) -> int:
    ...
    """
    if type(VALORES[carta]) == tuple and len(VALORES[carta]) == 2: # type: ignore
        if valor_minimo:
            return int(min(VALORES[carta])) # type: ignore
        else:
            return int(max(VALORES[carta])) # type: ignore
    else:
        return int(VALORES[carta]) # type: ignore
